...
class JobWorker:
    ...
    async def _process_job(self, job: Job) -> None:
        ...
        async with async_session_maker() as session:
            try:
                ...
                await self._retry_or_fail(job, "JOB_EXECUTION_TIMEOUT", message, ctx=ctx, request=request)
                ...
                await self._retry_or_fail(job, "DATASET_NOT_FOUND", str(e), ctx=ctx, request=request)
                ...
                await self._retry_or_fail(job, "ANALYTICS_ERROR", str(e), ctx=ctx, request=request)
                ...
                await self._retry_or_fail(job, "UNEXPECTED_ERROR", f"Unexpected error: {e}", ctx=ctx, request=request)
    ...
    async def _retry_or_fail(self, job: Job, error_code: str, error_message: str, *, ctx: TenantContext, request: AnalyticsRequest) -> None:
        non_retryable = {
