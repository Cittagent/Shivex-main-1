...
from src.utils.localization import currency_symbol
...
def idle_cost_bar(rows: list[dict], currency: str | None = None) -> str | None:
    if not rows:
        return None
...
def overconsumption_cost_bar(rows: list[dict], currency: str | None = None) -> str | None:
    if not rows:
        return None
    labels = [_truncate(r.get("device_name") or r.get("device_id") or "Unknown") for r in rows]
