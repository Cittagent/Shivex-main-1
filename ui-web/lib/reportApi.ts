...
export async function submitConsumptionReport(
  ...
  const res = await apiFetch(`${REPORT_SERVICE_BASE}/energy/consumption`, {
    method: "POST",
...
export async function submitComparisonReport(
  ...
  const res = await apiFetch(`${REPORT_SERVICE_BASE}/energy/comparison`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(params),
  });
...
export async function getReportStatus(
  ...
  const res = await apiFetch(withTenantQuery(`${REPORT_SERVICE_BASE}/${reportId}/status`, tenantId));
  if (!res.ok) {
    throw new Error(await readReportApiError(res));
  }
  return res.json();
...
export async function getReportResult(
  ...
  const res = await apiFetch(withTenantQuery(`${REPORT_SERVICE_BASE}/${reportId}/result`, tenantId));
  if (!res.ok) {
    throw new Error(await readReportApiError(res));
  }
  return res.json();
