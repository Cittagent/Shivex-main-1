...
export async function runWasteAnalysis(params: WasteRunParams): Promise<WasteRunResponse> {
  const res = await apiFetch(`${WASTE_SERVICE_BASE}/analysis/run`, {
    method: "POST",
...
export async function getWasteStatus(jobId: string): Promise<WasteStatus> {
  const res = await apiFetch(`${WASTE_SERVICE_BASE}/analysis/${jobId}/status`);
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
}

export async function getWasteResult(jobId: string): Promise<unknown> {
  const res = await apiFetch(`${WASTE_SERVICE_BASE}/analysis/${jobId}/result`);
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
}

export async function getWasteDownload(jobId: string): Promise<{ download_url: string }> {
  const res = await apiFetch(`${WASTE_SERVICE_BASE}/analysis/${jobId}/download`);
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
}
...
export async function getWasteHistory(limit = 20, offset = 0): Promise<{ items: WasteHistoryItem[] }> {
  const res = await apiFetch(`${WASTE_SERVICE_BASE}/analysis/history?limit=${limit}&offset=${offset}`);
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
}
