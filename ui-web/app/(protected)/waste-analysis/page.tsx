...
export default function WasteAnalysisPage() {
  ...
  useEffect(() => {
    async function bootstrap() {
      try {
        const [ds, tariff, plantRows] = await Promise.all([
          getDevices(),
          getTariffConfig(),
...
const onDownload = useCallback(async (id: string) => {
    try {
      const d = await downloadWastePdf(id);
                    ...
                    <td className="py-2 pr-4 space-x-3">
                      {h.status === "completed" && (
                        <>
                          ...
                          <button className="text-gray-600 hover:text-gray-800" onClick={() => onViewResult(h.job_id)}>Quick Snapshot</button>
                        </>
                      )}
                    </td>
                  </tr>
      ...
      {resultData && (
        <div className="bg-white rounded-xl border p-5 space-y-4">
          ...
          <div className="text-sm text-gray-600">
            Total Waste Cost: {resultData.total_waste_cost != null ? formatCurrencyValue(Number(resultData.total_waste_cost), "INR") : "N/A"}
          </div>
