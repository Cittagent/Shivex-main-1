...
- environment used: {"analytics_url": "http://localhost:8003", "auth_url": "http://localhost:8090", "cert_python": "/Users/vedanthshetty/.pyenv/versions/3.11.9/bin/python3.11", "data_url": "http://localhost:8081", "device_url": "http://localhost:8000", "repo_root": "/Users/vedanthshetty/Desktop/GIT-Testing/FactoryOPS-Cittagent-Obeya-main", "reporting_url": "http://localhost:8085", "rule_url": "http://localhost:8002", "started_at": "2026-04-08T15:19:14.602738+00:00"}

## Findings
- severity=critical; module=financials; role=system; classification=confirmed product defect; production-blocking=True; root cause=Targeted financial consistency tests failed with exit code 2.
- severity=high; module=notifications; role=system; classification=confirmed product defect; production-blocking=False; root cause=Notification and settings regression tests failed with exit code 1.
- severity=high; module=notifications; role=system; classification=confirmed product defect; production-blocking=False; root cause=Notification and settings regression tests failed with exit code 1.
- severity=high; module=notifications; role=system; classification=confirmed product defect; production-blocking=False; root cause=Notification and settings regression tests failed with exit code 1.
- severity=critical; module=runtime; role=system; classification=confirmed product defect; production-blocking=True; root cause=Log sweep found runtime instability indicators in service logs.
...
- Multi-org isolation: PASS | Org isolation validator passed.
...
- Role scoping: PASS | Org isolation validator passed.
- Machines page: NOT_EXECUTED | Not executed by this run.
- Machine detail page: NOT_EXECUTED | Not executed by this run.
- Rules: PASS | Rule creation accepted selected-device scope and per-rule recipients.
...
- Notification delivery intent: PASS | Telemetry triggered the rule and created a live alert artifact.
...
- Scheduled reports: PASS | Scheduled report reliability tests passed.
...
- Error handling: PASS | Org isolation validator passed.
...
- Final GO / NO-GO: FAIL | 7 checklist item(s) failed.

## Logs Review
- device-service: Unexpected log token(s): traceback, unhandled exception

## Production Recommendation
- NO-GO: 7 checklist item(s) failed.

## Follow-ups
- Not executed by this run.
- Not executed by this run.
- Notification and settings regression tests failed with exit code 1.
- Notification and settings regression tests failed with exit code 1.
- Notification and settings regression tests failed with exit code 1.
- Live energy report creation request failed.
- Not executed by this run.
- Targeted financial consistency tests failed with exit code 2.
- Not executed by this run.
- Not executed by this run.
